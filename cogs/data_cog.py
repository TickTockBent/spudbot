...
class DataCog(commands.Cog):
    ...
    def convert_data(self, raw_data):
        ...
        epoch_subsidy = int(raw_data.get('epochSubsidy', 0)) / 1e15
        processed['epochSubsidy'] = round(epoch_subsidy / 1e6, 2)
        debug_print('epochSubsidy', processed['epochSubsidy'], 
                    f"(raw: {raw_data.get('epochSubsidy', 0)} Smidge)")

        circ_supply = int(raw_data.get('circulatingSupply', 0)) / 1e15
        processed['circulatingSupply'] = round(circ_supply / 1e6, 2)
        debug_print('circulatingSupply', processed['circulatingSupply'], 
                    f"(raw: {raw_data.get('circulatingSupply', 0)} Smidge)")

        rewards = int(raw_data.get('rewards', 0)) / 1e15
        processed['rewards'] = f"{round(rewards / 1e6, 2)}M"
        ...
        processed['marketCap'] = round(market_cap / 1e6, 2)
        ...
        vested = int(raw_data.get('vested', 0)) / 1e15
        processed['vested'] = round(vested / 1e6, 2)
